...
export default function ListingDetailPage() {
  ...
  const fetchListing = async () => {
    try {
      const url = `${process.env.NEXT_PUBLIC_API_URL}/listings/${params.id}`;
      
      const data = await fetchWithTimeout<Listing>(url, {
        timeout: 10000,
        retries: 3,
      });
      ...
      const errorMsg = err instanceof Error ? err.message : 'Объявление не найдено';
      setError(errorMsg);
      ...
      <div className="mt-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          {keys.map((key) => {
            ...
            const value = typeof attrs[key] === 'boolean' ? (attrs[key] ? 'Да' : 'Нет') : attrs[key];

            return (
              <div key={key} className="flex justify-between py-2 px-3 bg-[#F9FAFB] rounded-lg">
                <span className="text-sm text-[#6B7280]">{label}</span>
                <span className="text-sm font-medium text-[#111827]">{sanitizeText(String(value))}</span>
              </div>
            );
          })}
        </div>
  ...
  if (error || !listing) {
    return (
      <div className="container-custom py-12 text-center">
        ...
        <p className="text-[#6B7280] mb-4">{error || 'Возможно, оно было удалено или перемещено'}</p>
  ...
  return (
    <div className="container-custom py-8">
      <div className="flex items-center gap-2 text-sm text-[#6B7280] mb-4">
        ...
        <span>/</span>
        <Link href="/listings" className="hover:text-[#111827]">
          Объявления
        </Link>
        <span>/</span>
        <span className="text-[#111827]">{sanitizeText(listing.title)}</span>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-xl border border-[#E5E7EB] p-6 sticky top-20">
            ...
            {/* Title and Price */}
            <h1 className="text-2xl font-bold text-[#111827] mb-2">{sanitizeText(listing.title)}</h1>
            <div className="text-3xl font-bold text-[#111827] mb-4">{formatPrice(listing.price)}</div>
            ...
            <div className="space-y-3 text-sm">
              {listing.description ? (
                <div>
                  ...
                  <div className="text-[#6B7280] mt-1 whitespace-pre-wrap break-words">
              ...
              {listing.address ? (
                <div>
                  ...
                  <p className="text-[#6B7280] mt-1">{sanitizeText(listing.address)}</p>
                </div>
              ) : null}
              ...
              <div>
                ...
                <p className="text-[#6B7280] mt-1">
                  ...
                  {listing.author.isSeller ? <span className="ml-2 text-xs text-[#6366F1]">Продавец</span> : null}
                </p>
                {listing.author.companyName ? (
                  <p className="text-xs text-[#9CA3AF]">{sanitizeText(listing.author.companyName)}</p>
                ) : null}
              </div>
