...
import { Shield, AlertTriangle, CheckCircle, Loader2, Car, FileText } from 'lucide-react';
...
export default function VehicleCheck({ vin, className = '' }: VehicleCheckProps) {
  ...
  const handleCheck = async () => {
    ...
    setLoading(true);
    try {
      // Здесь будет реальный API запрос к сервису проверки
      // Пока демо-данные
      ...
      setResult({
        ...
        title: Math.random() > 0.5 ? '✅ Автомобиль чист' : '⚠️ Есть ограничения',
