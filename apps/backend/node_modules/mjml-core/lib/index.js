...
var _formatHtml = require("./helpers/formatHtml");
...
var _skeletonLoader = _interopRequireDefault(require("./node-only/skeleton-loader"));
...
function getTemplateDelimiterRecoveryMessage(contextName) {
  if (typeof contextName === 'string' && contextName.toLowerCase() === 'css minification') {
...
function sanitizeTemplateVariablesInHtml(html, shouldSanitize, syntaxes, allowMixedSyntax, contextName) {
  ...
  if (broken.length) {
    ...
    throw new Error(`Unbalanced template delimiters found in CSS: ${details}. ${getTemplateDelimiterRecoveryMessage(contextName)}`);
  }
...
async function mjml2html(mjml, options = {}) {
  ...
  if (isNode && typeof options.skeleton === 'string') {
    const loadSkeleton = _skeletonLoader.default.default || _skeletonLoader.default.loadSkeleton || _skeletonLoader.default;
      ...
      for (const p of pathsArr) {
        if (fs.existsSync(p)) {
          ...
          if (isRootDir) {
            ...
            console.warn(`[MJML security] includePath "${p}" is the root directory, which is unsafe. Consider scoping includePath to a project templates folder.`);
          }
        }
      }
    }
  ...
  if (minify) {
    ...
    content = content.replace(/<!--\s*htmlmin:ignore\s*-->([\s\S]*?)<!--\s*htmlmin:ignore\s*-->/g, (_, inner) => {
      const token = `MJMLHTMLIGNORE${htmlminIgnoreList.length}END`;
      htmlminIgnoreList.push(inner);
      return token;
    });
    const sanitizationResult = sanitizeTemplateVariablesInHtml(content, sanitizeStyles === true && cssMinifyEnabled, syntaxes, allowMixedSyntax, 'CSS minification');
    ...
    if (sanitizationResult.didSanitize) {
      content = restoreTemplateVariablesInHtml(content, sanitizationResult.variableMap, sanitizationResult.propMap, sanitizationResult.isBlockVariable, syntaxes);
    }

    // Restore the htmlmin:ignore-protected chunks
    if (htmlminIgnoreList.length > 0) {
      content = content.replace(/MJMLHTMLIGNORE(\d+)END/g, (_, i) => htmlminIgnoreList[parseInt(i, 10)]);
    }
  } else if (beautify) {
    ...
    const sanitizationResult = sanitizeTemplateVariablesInHtml(content, sanitizeStyles === true, syntaxes, allowMixedSyntax, 'beautification');
    content = sanitizationResult.content;
    content = (0, _formatHtml.formatHtml)(content);
    if (sanitizationResult.didSanitize) {
      content = restoreTemplateVariablesInHtml(content, sanitizationResult.variableMap, sanitizationResult.propMap, sanitizationResult.isBlockVariable, syntaxes);
    }
  }
